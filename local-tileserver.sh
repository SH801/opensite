...
if command -v $COMMAND_NAME >/dev/null 2>&1; then
    ...
    if [ -n "${BUILD_FOLDER+1}" ]; then
        docker run --name openwindenergy-tileserver -d --rm -v "$BUILD_FOLDER"tileserver/:/data -p 8080:8080 --public_url ${TILESERVER_URL} maptiler/tileserver-gl --config config.json
    else
        docker run --name openwindenergy-tileserver -d --rm -v $(pwd)/build/tileserver/:/data -p 8080:8080 --public_url ${TILESERVER_URL} maptiler/tileserver-gl --config config.json
    fi
fi