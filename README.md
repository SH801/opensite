...
```
git clone https://github.com/SH801/opensiteenergy
cd opensiteenergy
./build-docker.sh
```
...
![Open Wind Energy Site Constraints Map](/opensiteenergy-image-map.png)

This map is also available at [https://map.opensite.energy](https://map.opensite.energy). 
...
![Open Wind Energy Custom Configuration - Scotland](/opensiteenergy-image-custom-scotland.png)
...
- **Specify configuration name**: Specify *name* of configuration from list of available configurations on [Open Wind Energy Configuration Area](https://data.opensite.energy/group/custom). The system will then download the configuration file with the same title from the data portal.
...
./build-cli.sh --custom configuration/custom-opensiteenergy.yml
./build-docker.sh --custom configuration/custom-opensiteenergy.yml
```

To use a custom configuration file on the internet:
```
./build-cli.sh --custom https://yourdomain.org/custom-opensiteenergy.yml
./build-docker.sh --custom https://yourdomain.org/custom-opensiteenergy.yml
```

To use a specific configuration from the [Open Wind Energy Configuration Area](https://data.opensite.energy/group/custom):
```
...
# Specific datasets from data.opensite.energy to use
# Note: any datasets must exist on data.opensite.energy to be used
structure:
...
# Note: any datasets must exist on data.opensite.energy to be buffered
buffers: 
  hedgerows--uk: 
    50
  inland-waters--uk:
...
- `TILESERVER_URL`: URL of [TileServer GL](https://github.com/maptiler/tileserver-gl) instance where you will host your mbtiles, eg. `https://tiles.opensite.energy`. This variable is used when creating the MapLibre-GL test site in `[build-directory]/app/index.html` and the related TileServer-GL `*.json` style files in `[build-directory]/tileserver/styles/`.
...
The toolkit downloads the latest definition of onshore wind site constraints, including recommended buffers, from the Open Wind Energy [CKAN](https://ckan.org/) open data portal [data.opensite.energy](https://data.opensite.energy). 
...
```
 ...
 The toolkit then amalgamates tables by *group*, as defined in the Open Wind Energy open data portal [data.opensite.energy](https://data.opensite.energy) (see, for example, the group [Landscape and visual impacts](https://data.opensite.energy/group/landscape-and-visual-impacts)). 
...
The following custom fields are used within the [CKAN](https://ckan.org/) [Open Wind Energy Data Portal](https://data.opensite.energy) to describe how datasets should be processed by the Open Wind Energy toolkit:
